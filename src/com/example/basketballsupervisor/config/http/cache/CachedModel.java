 ...
 * 
 * To save an object to the cache use {@link #save(ModelCache)}, when updating
 * an object with any new data use {@link #reload(ModelCache)}, and when wanting
 * to find an object from the cache, use
...
public abstract class CachedModel implements Parcelable {
	 ...
	 * Generally this is the required used.
	 * 
	 * Overwritten when saving subclasses over the top of separate superclass
	 * cache stores. e.g.:
	 * 
	 * <pre>
	 * {@code
	...
	/**
	 * Attempts to save the object in the cache using a given key. Generally
	 * only used for saving subclasses over the top of separate superclass cache
	 * stores.
	 * 
	 * @param modelCache
	 *            Cache to save to.
	...
	public boolean reload(ModelCache modelCache) {
		...
		if ((modelCache != null) && (key != null)) {
			...
			if ((cachedModel != null)
					&& (cachedModel.transactionId > this.transactionId)) {
	 ...
	 * @return Whether or not an internal cached object was updated (useful for
	 *         optimization purposes, especially on lists).
	 */
